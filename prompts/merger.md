...
由 Agent 根据用户**意图**判断：**在已有交底书或上一轮输出上补充新材料**（新文档、新代码说明、粘贴片段、扩展章节等），且以**合并进现有结构**为主时，**应**执行本流程；用户按 **`disclosure_builder.md` §7.6** 仅要求**第 15 章保护点书式强化**、已说明侧重点者，同样走本流程（合并范围以第 15 章为主）。**不要求**用户说出「迭代」等固定词；也**不必**先询问是否进入迭代模式。
...
4. **一致性**：合并后执行 `disclosure_self_check.md` 中的 **8.2、8.3** 快速检查；若涉及第 11 章公式、关键技术参数或实施例，须同步核对 **`disclosure_builder.md` §7.7**（符号定义、维度下标、关键参数同形）。
