...
本步骤是整篇交底书的前置门禁：重点是**检索查重**，找到最接近的已有方案，坚决**避开已有专利点**，再基于差异化提出建议、提出**反问**。若本行业内已有专利覆盖度高，可借助**其他行业类似专利**的解决路径，迁移出可保护的**新的金点子**。后续交底书正文必须精简不冗长，写清楚专利逻辑思路，并**严格遵守 YH 15 项**。
...
- **其他行业类似专利启发**：当本行业内结果过密或创新空间受限时，可补充检索其他行业类似专利，但必须说明可迁移的技术逻辑，不能强行套概念。
