...
> **C 阶段步骤 4 按需加载**。执行时遵照 `patent_points_analyzer.md` 执行清单第 4-5 项；本文件为检索操作细则。

## 核心目标

本步骤是整篇交底书的前置门禁：**检索查重**，找到最接近的已有方案，坚决**避开已有专利点**，再提出**反问**。可借助**其他行业类似专利**的解决路径挖掘**新的金点子**。正文须**严格遵守 YH 15 项**。
...
4. **执行方式**（C 阶段读完本文件后**先尝试**）：
   ...
   - **合并责任在 Agent**：每次调用解析 **stdout** 上**唯一一行** **`EPUB_HITS_JSON:`** 后的 JSON 数组；在推理中按 **`pub_number`** 为主键去重合并（无则 **`link`**，再否则可用标题前缀），得到**一份**总表后，在对话中汇总检索结果，再写入 1.1。
   ...
   - 将 JSON 中**可核验**的公开号、标题、**国知局站点内详情链接**在对话中汇总，并写入 1.1（见下 **`abstract` 必用**）。
...
5. **`abstract` 字段（国知局条目，规定必用）**
   ...
   - **必用**：交底书 **1.1** 中对该专利的**技术方案概括、应用场景与局限性分析**，**必须先基于对该 `abstract` 的完整阅读与理解**后再撰写；**禁止**仅凭标题、公开号或 URL **臆造**方案要点或与摘要矛盾的表述。
   - **充分理解**：在写入 1.1 前，Agent 须在**推理过程内**明确：摘要所涉**技术领域、解决什么问题、核心手段/模块、主要效果或流程**；若摘要与标题存在差异，**以摘要为准**概括该技术。
   - **正文呈现**：交底书 1.1 中**不得**大段逐字粘贴官方摘要（避免抄袭与超字数）；应**消化后**用**自己的话**压缩为「方案概括 + 应用 + 缺点/局限」；对话中可给出稍长的摘录供用户核对，但须标注来源于公布站摘要。
   - **缺失时**：若某条 JSON **无** `abstract` 或为空（旧版页面 / 表格布局未解析到等），须在对话中向用户说明「该条无摘要字段」，并改用**详情页**或 **Google Patents** 等可核验来源补全理解后再写 1.1，**不得**留空理由含糊带过。
...
- 应用场景
- **局限性**
- **与本案相似点与规避路径（写入交底书第 8 章 1.1 时必填）**：须指出（a）该专利哪些技术特征与本案重叠或相似；（b）本案通过什么差异化路径可以绕开（如实现机制不同、架构不同、维度不同、技术路径不同等）；（c）必要时借用其他行业类似专利的解题逻辑（如自动驾驶的多传感器后融合、NLP 的特征哈希等）提供绕行思路。此要求与 `template_reference_yh.md` 第 8 章六要素一致。避让分析中须系统性地参照「专利点经典避让法」（见同目录 `patent_avoidance_methods.md` 十类方法），从要素省略/替代/变更/组合/领域转用/问题重定义/维度限定/闭环反馈/边界处理/跨模块协同等角度生成具体规避方向。
- **公开源 URL（必填）**：每一条必须附带**至少一个可公开访问、与著录项一致**的链接，在对话中汇总并写入交底书 1.1，便于代理人复核。**禁止编造或猜测 URL**；写入前应在浏览器中打开确认页面可访问且对应同一文献/专利。
...
查新结束后，除列出已有技术外，还必须输出：

- **应避开的已有专利点**：哪些技术手段、流程、结构或效果已经被相近专利覆盖，正文不得包装成自身创新点；
- **可继续挖掘的新方向**：基于本案材料与已有技术差异，提出 2–5 个可能形成新的金点子的方向；
- **反问发明人**：提出 2–5 个具体问题，确认是否存在未披露的数据闭环、控制策略、参数约束、异常处理、跨模块协同或跨行业迁移机制；
- **其他行业类似专利启发**：当本行业内结果过密或创新空间受限时，可补充检索其他行业类似专利，但必须说明可迁移的技术逻辑，不能强行套概念。此分析须落实为第 8 章 1.1 各条专利「与本案相似点与规避路径」中的跨行业类比（如自动驾驶的多传感器后融合、NLP 的特征哈希、推荐系统的协同过滤等），为本案提供具体的绕行思路——而非仅停留在概念层面。

## 记录习惯

便于写进交底书：保留专利号、标题、**消化摘要后的**一两句方案概括（有 **`abstract`** 时概括须可追溯至该摘要）；**每条另起一行给出「来源 URL」**，并附「与本案相似点与规避路径」。避免大段抄袭权利要求或整段粘贴官方摘要。交底书正文中所有内容以序号罗列，不使用表格。

### 1.1「检索说明」写法（交付正文，必遵）

写入交底书 **1.1** 开头的「检索说明」时，面向**代理人/审查员**表述，**不要**暴露 Agent 查新流程或本仓库工具实现。

- **须写**：实际使用的**公开数据库或渠道名称**（如「国家知识产权局专利公布公告系统」）、本案**主要检索词**（与交底书正文用词一致或概括）；若部分条目经 **Google Patents** 等公开页复核著录项，可一句带过。
...
Agent 在对话中可提及是否调用脚本、是否降级 WebSearch 等操作细节；**上述内容不得写入交底书 1.1**。
