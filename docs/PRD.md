...
Step 6  摘要预览与确认（prompts/disclosure_preview.md）
        ↓
Step 7  全文交底书（prompts/disclosure_builder.md + template_reference_yh.md；template_reference.md 保留旧版参考）
        ↓
...
- 交底书正文**不得**包含「自检清单」章节。
- 脱敏要求见 `disclosure_builder.md` / `template_reference_yh.md`；旧版细则保留在 `template_reference.md`。
- 查新结论须写入第一章并与技术问题、方案呼应；渠道与著录细则见 `prompts/prior_art_search.md`。
- 交底书定稿须**同时**交付 Markdown 与 Word；含图示时同级交付 `images/` 文件夹；文件名 **`{案件名}_{YYYYMMDDHHmmss}`**（§7.3 第 5 点，含首次与迭代）；降级使用 mermaid 时 `tools/mermaid_render.py` 默认调用 `md_to_docx.py`；Word 失败时允许人工补转（见 `tools/README.md`）。
