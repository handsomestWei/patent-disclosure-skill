...
- **图示定稿（Step 7）**：默认由 LLM 规划图示并用 **Images 2.0** 生成专利交底书风格黑白线条图，全部放入同级 **`images/`**；Images 2.0 不可用或质量不合格时可降级为 fenced **mermaid**，执行方式、**`mmdc`** 安装与降级规则见下表「交底书定稿交付」行及 **`tools/README.md`**。
...
| 交底书定稿交付（**须同时** .md + .docx） | 默认按 **`template_reference_yh.md`** 规划图示，LLM 生成 Images 2.0 提示词并产出黑白线条图，全部保存到同级 **`images/`**，正文按图号插入并配图注；Images 2.0 不可用或质量不合格时可降级为 fenced ``mermaid``。定稿执行 **`tools/mermaid_render.py`**：mermaid 转 PNG（失败块保留围栏）后默认生成同名 **.docx**；若 Word 失败，按 stderr 提示手动运行 **`md_to_docx.py`**。详见 **`tools/README.md`** |
...
| Step 7 | `prompts/disclosure_builder.md` + `prompts/template_reference_yh.md`（默认）+ `prompts/template_reference.md`（旧版参考） | 新版 15 项交底书结构、脱敏、Images 2.0 图示规范、**符号与公式体例（§7.7）**；旧版 mermaid 与符号/公式范例仍可在 `template_reference.md` 中参考 |
...
6. **`Read`** `disclosure_builder.md`、**`Read`** `template_reference_yh.md`，必要时再 **`Read`** 旧版 `template_reference.md` → 执行 Step 7（**首次交付**的 `.md`/`.docx` 亦须 **`{案件名}_{YYYYMMDDHHmmss}`**，§7.3 第 5 点；含图示时同级交付 `images/`）；交付对话中**须**按 **`disclosure_builder.md` §7.6** 补充「权利要求偏向点」建议交互（**仅对话**，不入正文）
...
- **补充材料 / 扩展章节**或 **§7.6 第 15 项权利要求书式强化（用户已声明侧重点）**：`Read` → `iteration_context.md` → `merger.md`；合并结果**另存为**带时间戳的 `.md`/`.docx`（§7.3 第 5 点）；**追加** `交底书修订对话记录.md`（`iteration_dialog_log.py` 或手工）；完成后**必须**输出「合并摘要」留档；若本轮亦为定稿交付，**仍建议**简短附带 §7.6 类引导
...
□ 除用户明确跳过外，完成摘要预览
□ 脱敏、Images 2.0 图示或降级 mermaid、章节引用符合 `template_reference_yh.md`；含图示时同级 `images/`、图号、图注、附图说明、附图标记一致；含公式时 **§7.7 体例**（维度下标、无字母多义、LaTeX 分隔符统一）及符号表/参数表同形已满足；**已交付 .md 与 .docx**，且**文件名符合 §7.3 第 5 点**（**凡交付均含**时间戳后缀）；**正文无**技能/示例仓库类文末脚注
□ 定稿类对话已含 **`disclosure_builder.md` §7.6**「权利要求偏向点」建议交互（默认围绕第 15 项，**不入正文**、**不捏造**未在稿内出现的保护取向）；迭代再走 merger 时见 **`iteration_context.md`** 表格补充行
